...
exports.vehicleInputDtoValidation = void 0;
const driver_1 = require("../types/driver");
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const vehicleInputDtoValidation = (data) => {
    const errors = [];
    console.log('validation data: ', data);
    const { name, phoneNumber, email, vehicleMake, vehicleFeatures, vehicleModel, vehicleYear, vehicleLicensePlate, vehicleDescription, } = data.attributes;
    if (!name ||
        typeof name !== 'string' ||
        name.trim().length < 2 ||
        name.trim().length > 15) {
    ...
    if (!Array.isArray(vehicleFeatures)) {
        ...
        const existingFeatures = Object.values(driver_1.VehicleFeature);
        if (vehicleFeatures.length > existingFeatures.length ||
...
exports.vehicleInputDtoValidation = vehicleInputDtoValidation;
